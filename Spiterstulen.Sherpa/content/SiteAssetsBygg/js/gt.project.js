...
GT.Project.PopulateProjectPhasePart = function () {
    SP.SOD.executeFunc('sp.js', 'SP.ClientContext', function () {
        GT.jQuery.when(GT.Project.GetPhaseNameFromCurrentItem()).then(function (phaseName) {
            ...
            var alternatePhases = {
                'Konsept': ['Mulighetsstudie (F0)','Programfase (F1)'],
                'Planlegge': ['Skisseprosjekt (F2)', 'Forprosjekt (F2)','Detaljprosjekt (F2)'],
                'Gjennomføre': ['Byggfase (F3', 'Sluttfase og overtakelse'],
                'Avslutte': ['Prøvedrift'],
                'Realisere': ['Forvaltning','Vedlikehold'],
            };
            for (var ix = 0; ix < phases.length; ix++) {
                GT.jQuery('.projectPhases').append(GT.Project.GetPhaseLogoMarkup(phases[ix], alternatePhases[phases[ix]], phases[ix] == phaseName, true, true, ix));
            }
        });
    });
};
...
GT.Project.GetPhaseLogoMarkup = function (phaseName, alternatePhaseName, selected, wrapInListItemMarkup, linkToDocumentLibrary, index) {
    ...
    var markup = '<div class="gt-phaseIcon ' + selectedClass + '">' +
		...
		'<span class="projectPhase alternatePhase"> (' + alternatePhaseName.join(', ') + ')</span>' : "") +
        '</div>';
